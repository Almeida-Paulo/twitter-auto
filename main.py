...
if entries:
    ...
    if second_guid:
        cursor.execute("SELECT guid FROM guid_list WHERE guid = %s", (second_guid,))
        already_posted = cursor.fetchone() is not None
        if not already_posted:
            try:
                client.create_tweet(text = f'{second_excerpt} {second_link}')
                time.sleep(5) # espera 5 segundos antes de enviar outro tweet
            except Exception as e:
                if '429' in str(e):
                    print("Muitas solicitações. Espere um momento antes de tentar novamente.")
...
scheduler.add_job(check_feed, 'interval', minutes=25)
scheduler.start()
